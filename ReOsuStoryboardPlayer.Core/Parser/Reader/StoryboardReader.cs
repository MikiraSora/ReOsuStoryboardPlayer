...
namespace ReOsuStoryboardPlayer.Core.Parser.Reader
{
    ...
    {
        private readonly ParallelOptions parallel_options = new ParallelOptions
            {MaxDegreeOfParallelism = Setting.UpdateThreadCount};
        ...
        public StoryboardObject ParseObjectLine(string line)
        {
            ...
            if (!Enum.TryParse<StoryboardObjectType>(data_arr[0], true, out var obj_type) ||
                !(obj_type == StoryboardObjectType.Background || obj_type == StoryboardObjectType.Animation ||
                  obj_type == StoryboardObjectType.Sprite))
            {
            ...
            if (!(obj is StoryboardBackgroundObject))
            {
                ...
                obj.ImageFilePath =
                    StringPool.Shared.GetOrAdd(data_arr[3].Trim().Trim('\"').Replace("/", "\\").ToLower());

                var baseX = data_arr[4].ToSigle();
                var baseY = data_arr[5].ToSigle();
                var basePosition = new Vector(baseX, baseY);
                obj.BaseTransformResetAction += x => x.Postion = basePosition;
            ...
            else
            {
                ...
                obj.ImageFilePath =
                    StringPool.Shared.GetOrAdd(data_arr[2].Trim().Trim('\"').Replace("/", "\\").ToLower());

                var baseX = data_arr[3].ToSigle();
                var baseY = data_arr[4].ToSigle();

                var position = data_arr.Length > 4
                    ? new Vector(baseX, baseY)
                    : Vector.Zero;
        ...
        {
            var dot_position = animation.ImageFilePath.LastIndexOf('.');
            animation.FrameFileExtension = StringPool.Shared.GetOrAdd(animation.ImageFilePath.Substring(dot_position));
            animation.FrameBaseImagePath =
                StringPool.Shared.GetOrAdd(animation.ImageFilePath.Replace(animation.FrameFileExtension, string.Empty));
        ...
        {
            var list =
                lines.Count >= Setting.ParallelParseCommandLimitCount && Setting.ParallelParseCommandLimitCount != 0
        ...
        public List<Command> ParseCommands(List<string> lines, long base_line)
        {
            List<Command> commands = new List<Command>(),
                cur_group_cmds = ObjectPool<List<Command>>.Instance.GetObject();
        ...
        {
            var result_list = new ConcurrentBag<(long index, Command[] cmds, bool is_sub)>();
            ...
            var result = result_list.SelectMany(p => p.cmds.Select(cmd => (p.index, cmd, p.is_sub)))
                .OrderBy(z => z.index);
            var sub_cmds = result.Where(x => x.is_sub);
            var groups = result.Where(x => x.cmd is GroupCommand && !x.is_sub).Select(x => x.cmd)
                .OfType<GroupCommand>();
